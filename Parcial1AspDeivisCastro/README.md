...
```html
<asp:TextBox ID="txtCorreoRecuperacion" runat="server" TextMode="Email" />
<asp:RequiredFieldValidator runat="server" ControlToValidate="txtCorreoRecuperacion"
    ErrorMessage="El correo es obligatorio" ForeColor="Red" Display="Dynamic" />
<asp:RegularExpressionValidator runat="server" ControlToValidate="txtCorreoRecuperacion"
    ValidationExpression="\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*"
    ErrorMessage="Formato de correo inválido" ForeColor="Red" Display="Dynamic" />

<asp:Button ID="btnContinuar" runat="server" Text="Continuar" OnClick="btnContinuar_Click" />

<asp:Label ID="lblErrorRecuperacion" runat="server" ForeColor="Red" Visible="false" />
<asp:Label ID="lblConfirmacion" runat="server" ForeColor="Green" Visible="false" />
```
